...
type downloadPool struct {
	...
	maxWorkers int
	pending   int // downloads starting but not yet registered in active
}
...
type Model struct {
	...
	// config modal
	configInputs [5]textinput.Model
	configFocus  int
...
func NewModel(repo *library.Repository, cfg *config.Config) Model {
	...
	maxWorkers := cfg.DownloadWorkers
	if maxWorkers < 1 {
		maxWorkers = 1
	}
	if maxWorkers > 10 {
...
func (m Model) Init() tea.Cmd {
	return tea.Batch(refreshSongsCmd(m.repo), loadQueuedSongsCmd(m.repo), m.spinner.Tick)
}
